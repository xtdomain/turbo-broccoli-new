...
+ Если шаблон - добавить: ```$content->addRoute()```, а также перед html тегами - ```$content->startRoute('название шаблона')```.
  + Нет обязательных требований: в целом слой это верстка html с возможностью получать динамичное содержимое (массив $pageData['имя'] - его параметры задаются в контроллере, пример - $this->pageData['имя'], и приравниваем к методу модели, см. пример интернет-магазина). Однако если это шаблон, и в нем будет отражаться множество видов, то обязательно необходимо:
    + вставить имя шаблона (контроллера) $content->startRoute('название шаблона'),
...
+ В контроллере:
 ...
 ```
$pagination = $this->model->Pagination('category'); // Подключение пагинации (прописываем 'имя контроллера')
